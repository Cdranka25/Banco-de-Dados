...
-- Atividade SQL - Prática do SELECT com múltiplas tabelas
...
SELECT p.nm_proprietario, p.ds_logradouro, l.nm_localidade
...
FROM marca m, modelo mo
WHERE m.ds_marca = 'FIAT'
...
SELECT m.ds_marca, mo.ds_modelo
FROM marca m, modelo mo
ORDER BY m.ds_marca, mo.ds_modelo
...
SELECT v.*, m.ds_marca, mo.ds_modelo, v.nr_ano_mod, c.ds_combustivel
FROM veiculo v, marca m, modelo mo, combustivel c
WHERE v.nr_placa IS NOT NULL


-- g) Listar os veículos (com placa, descrição da marca, descrição do modelo) que possuem como acessório "Direção hidráulica";
SELECT v.nr_placa, m.ds_marca, mo.ds_modelo, a.ds_acessorio
FROM veiculo v, acessorio a, marca m, modelo mo
WHERE a.ds_acessorio = 'Direção Hidráulica'


-- h) Listar dados completos dos veículos: placa, descrição da marca, descrição do modelo, descrição da cor, ano modelo, ano fabricação, quantidade de km rodado, quantidade de portas, 
